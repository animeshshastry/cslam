...
RGBDHandler<SyncPolicy>::RGBDHandler(rclcpp::Node * node) : SensorHandler(node) 
{
    ...
    //     qos);
    sub_image_color_.subscribe(node_, node_->get_parameter("frontend.color_image_topic").as_string(), "raw");
    sub_image_depth_.subscribe(node_, node_->get_parameter("frontend.depth_image_topic").as_string(), "raw");
    sub_camera_info_color_.subscribe(node_, node_->get_parameter("frontend.color_camera_info_topic").as_string());
