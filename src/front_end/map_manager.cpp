...
void MapManager::process_new_sensor_data()
{
  ...
  if (keyframe_generation_ratio_threshold_ < 0.99f && keyframe_generation_ratio_threshold_ > 0.001f && 
    ...
    if (!t.isNull())
    {
      ...
      {
        RCLCPP_DEBUG(get_logger(), "Keeping OF KF due to high number of inliers from pervious KF %d %f", reg_info.inliers,
                        inliersRatio);
    ...
    if (odom_recovery_state == RECOVERY_FAILED) {
      ...
      if ( !t.isNull()) {
        ...
        if (inliersRatio > keyframe_generation_ratio_threshold_ )
        {
          ...
          RCLCPP_DEBUG(get_logger(), "After feature matching -- frame not close to prev kf %d %f. Making new KF", reg_info.inliers,
                inliersRatio);
        }
      } else {
        lastKFPose.setIdentity();
